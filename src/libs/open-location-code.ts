...
export default class OpenLocationCode {
  ...
  public static encode(latitude: number, longitude: number, codeLength: number = OpenLocationCode.CODE_PRECISION_NORMAL): string {
    ...
    let code = OpenLocationCode.encodePairs(latitude, clippedLongitude, Math.min(codeLength, OpenLocationCode.PAIR_CODE_LENGTH_));
    // If the requested length indicates we want grid refined codes.
    if (codeLength > OpenLocationCode.PAIR_CODE_LENGTH_) {
      code += OpenLocationCode.encodeGrid(clippedLatitude, longitude, codeLength - OpenLocationCode.PAIR_CODE_LENGTH_);
    }
    return code;
  };
